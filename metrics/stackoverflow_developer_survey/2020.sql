-- DuckDB: Calculates web framework popularity, loved/wanted/dreaded metrics from Stack Overflow 2020 survey data.
create table results_2020 as WITH frameworks AS (
    SELECT UNNEST([
        'Angular',
        'React',
        'Next',
...
current_popularity AS (
    ...
    FROM
        ...
        frameworks
    WHERE
        lower(WebFrameWorkedWith) LIKE '%' || lower(framework) || '%'
    GROUP BY
        framework
),
desired_popularity AS (
    ...
    FROM
        ...
        frameworks
    WHERE
        lower(WebFrameDesireNextYear) LIKE '%' || lower(framework) || '%'
    GROUP BY
        framework
),
loved_count AS (
...
wanted_count AS (
    ...
    FROM
        ...
        frameworks
    WHERE
        not(lower(WebFrameWorkedWith) LIKE '%' || lower(framework) || '%')
        and
        lower(WebFrameDesireNextYear) LIKE '%' || lower(framework) || '%'
    GROUP BY
        framework
...
dreaded_count AS (
    ...
    WHERE
        lower(WebFrameWorkedWith) LIKE '%' || lower(framework) || '%'
        and
        not(lower(WebFrameDesireNextYear) LIKE '%' || lower(framework) || '%')
    GROUP BY
        framework
)
SELECT
    ...
    Current_Popularity / 42279 as Current_Popularity_Percent,
    coalesce(d.Popularity, 0) AS Desired_Popularity,
    Desired_Popularity / 42279 as Desired_Popularity_Percent,
    ...
    Wanted / 42279 as Wanted_Percent,
